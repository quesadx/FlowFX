 ...
 * @author quesadx
 */
public class PersonSelectionController extends Controller implements Initializable {
    ...
    private ObservableList<PersonDTO> masterData = FXCollections.observableArrayList();
    ...
    private void onActionBtnSearch(ActionEvent event) {
        ...
        ObservableList<PersonDTO> filtered = FXCollections.observableArrayList();
        for (PersonDTO p : masterData) if (matchesFilter(p, f)) filtered.add(p);
        tvPersons.setItems(filtered);
    }

    @Override
    public void initialize() { /* secondary init unused */ }
    ...
    private void configureTable() {
        if (tbcPersonName.getCellValueFactory() == null)
            tbcPersonName.setCellValueFactory(new PropertyValueFactory<>("firstName"));
        if (tbcPersonLastName.getCellValueFactory() == null)
            tbcPersonLastName.setCellValueFactory(new PropertyValueFactory<>("lastName"));
        if (tbcPersonId.getCellValueFactory() == null)
    ...
    private boolean matchesFilter(PersonDTO p, String filter) {
        ...
        if (String.valueOf(p.getId()).toLowerCase(Locale.ROOT).contains(filter)) return true;
        if (p.getFirstName() != null && p.getFirstName().toLowerCase(Locale.ROOT).contains(filter)) return true;
        if (p.getLastName() != null && p.getLastName().toLowerCase(Locale.ROOT).contains(filter)) return true;
        return false;
    }
