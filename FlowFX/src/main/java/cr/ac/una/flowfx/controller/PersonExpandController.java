 ...
 * @author quesadx
 */
public class PersonExpandController extends Controller implements Initializable {

    @FXML private AnchorPane root;
    @FXML private VBox vbCover;

    @FXML private MFXButton btnReturnPersonSignUp;
    ...
    private void bindFields() {
        ...
        txfPersonLastName.textProperty().bindBidirectional(vm.lastNameProperty());
        txfPersonMail.textProperty().bindBidirectional(vm.emailProperty());
        txfPersonUsername.textProperty().bindBidirectional(vm.usernameProperty());
        psfPersonPassword.textProperty().bindBidirectional(vm.passwordProperty());

        // ID numeric sync (manual two-way to convert long<->String)
        txfPersonId.textProperty().addListener((obs, o, n) -> {
        ...
        vm.idProperty().addListener((obs, o, n) -> {
            String nv = (n == null || n.longValue() == 0L) ? "" : String.valueOf(n.longValue());
        ...
        cbIsActive.setSelected(vm.getStatus() != null && Character.toUpperCase(vm.getStatus()) == 'A');
        cbIsActive.selectedProperty().addListener((obs, o, sel) -> vm.setStatus(sel ? 'A' : 'I'));
        vm.statusProperty().addListener((obs, o, n) -> {
        ...
        cbIsAdmin.setSelected(vm.getIsAdmin() != null && Character.toUpperCase(vm.getIsAdmin()) == 'Y');
        cbIsAdmin.selectedProperty().addListener((obs, o, sel) -> vm.setIsAdmin(sel ? 'Y' : 'N'));
        vm.isAdminProperty().addListener((obs, o, n) -> {
